          ...
          {navItems.map(({ href, Icon, label }) => {
            ...
            return (
              <Link
                key={label}
                href={href}
                className={`flex items-center space-x-3 mb-5 group p-2 rounded-md transition-all ${
                  isActive ? "bg-[#F79B72] mr-5" : "hover:bg-[#F79B72] hover:mr-5"
                }`}
              >
                <Icon
                  className={`h-8 w-8 ${
                    isActive ? "text-[#2A4759]" : "text-[#F79B72] group-hover:text-[#2A4759]"
                  }`}
                />
                <h1
